...
async function loadAuctionBids(auctionId){
  const{data,error}=await sb.from('bids').select('*').eq('auction_id',auctionId).order('created_at',{ascending:false}).limit(20);
  if(error){console.error('loadAuctionBids error:',error);toast(db.lang==='pl'?'Błąd wczytywania historii ofert':'Error loading bid history');return[];}
  return data||[];
}
...
async function placeBid(auctionId,amount){
  ...
  const bidderNick=db.profile?.nick||currentUser.email;
  const{data,error}=await sb.rpc('place_bid',{p_auction_id:auctionId,p_amount:amount,p_bidder_nick:bidderNick});
  if(error){toast(error.message||(db.lang==='pl'?'Błąd składania oferty':'Error placing bid'));return false;}
  const clubLabel=[data?.club,data?.season].filter(Boolean).join(' ');
  // Powiadom poprzedniego najwyższego licytującego (jeśli inny niż Ty)
  if(data?.previous_bidder_id&&data.previous_bidder_id!==currentUser.id){
    await createNotification(data.previous_bidder_id,'outbid',db.lang==='pl'?'Zostałeś przebity!':'You have been outbid!',db.lang==='pl'?`Twoja oferta na ${clubLabel} została przebita. Aktualna cena: ${amount.toFixed(2)} PLN`:`Your bid on ${clubLabel} has been outbid. Current price: ${amount.toFixed(2)} PLN`,`auction:${auctionId}`);
  }
  // Powiadom sprzedającego
  if(data?.seller_id){
    await createNotification(data.seller_id,'bid',db.lang==='pl'?'Nowa oferta!':'New bid!',db.lang==='pl'?`${bidderNick} złożył ofertę ${amount.toFixed(2)} PLN na ${clubLabel}`:`${bidderNick} placed a ${amount.toFixed(2)} PLN bid on ${clubLabel}`,`auction:${auctionId}`);
  }
...
async function buyNow(auctionId){
  if(!currentUser){toast(db.lang==='pl'?'Musisz być zalogowany':'You must be logged in');return;}
  const buyerNick=db.profile?.nick||currentUser.email;
  const{data,error}=await sb.rpc('buy_now_auction',{p_auction_id:auctionId,p_buyer_nick:buyerNick});
  if(error){toast(error.message||(db.lang==='pl'?'Błąd zakupu':'Purchase error'));return;}
  const clubLabel=[data?.club,data?.season].filter(Boolean).join(' ');
  if(data?.seller_id){
    await createNotification(data.seller_id,'sale',db.lang==='pl'?'Koszulka sprzedana!':'Jersey sold!',db.lang==='pl'?`${buyerNick} kupił ${clubLabel} za ${data.price} PLN`:`${buyerNick} bought ${clubLabel} for ${data.price} PLN`,`auction:${auctionId}`);
  }
...
function startAuctionTimer(id,endsAt,elId){
  ...
  auctionTimers[id]=setInterval(()=>{
    ...
    el.textContent=txt;
    const diff=new Date(endsAt)-new Date();
    el.style.color=diff<60000?'#f87171':diff<300000?'#f59e0b':'rgba(255,255,255,.92)';
...
function bAuctions(){
  ...
  <div class="fx-b mb12 s1">
    ...
    <div class="fx-c gap8">
      ...
      ${adminMode?`<button class="btn btn-d" onclick="adminClearEndedAuctions()">${icon('trash',12,"display:inline-block;vertical-align:-2px;margin-right:5px")}${db.lang==='pl'?'Wyczyść zakończone':'Clear ended'}</button>`:''}
...
function buildAuctionCard(a){
  ...
  return `<div class="mkt-card" data-auction-id="${a.id}" style="border-color:${isEnded?'var(--border)':'rgba(239,68,68,.25)'};background:${isEnded?'var(--bg2)':'rgba(239,68,68,.03)'}">
    <div class="shirt-photo" style="background:var(--bg3)">
      ...
      ${!isEnded?`<div style="position:absolute;top:8px;left:8px;background:rgba(239,68,68,.9);border-radius:6px;padding:3px 8px;font-family:'JetBrains Mono',monospace;font-size:8px;color:#fff;display:flex;align-items:center;gap:4px"><span style="width:6px;height:6px;border-radius:50%;background:#fff;animation:pulse 1s ease-in-out infinite;flex-shrink:0"></span>LIVE</div>`:`<div style="position:absolute;top:8px;left:8px;background:rgba(20,33,61,.85);border-radius:6px;padding:3px 8px;font-family:'JetBrains Mono',monospace;font-size:8px;color:rgba(255,255,255,.6)">${a.status==='sold'?(db.lang==='pl'?'SPRZEDANA':'SOLD'):a.status==='cancelled'?(db.lang==='pl'?'ZAKOŃCZONA PRZED CZASEM':'ENDED EARLY'):(db.lang==='pl'?'ZAKOŃCZONA':'ENDED')}</div>`}
      ...
      ${!isEnded?`<div style="position:absolute;bottom:8px;left:8px;right:8px;background:rgba(20,33,61,.9);border-radius:8px;padding:5px 8px;display:flex;justify-content:space-between;align-items:center;gap:6px;overflow:hidden">
        <div style="font-family:'JetBrains Mono',monospace;font-size:8px;color:rgba(255,255,255,.65);letter-spacing:.04em;flex-shrink:0">${db.lang==='pl'?'KOŃCZY SIĘ':'ENDS IN'}</div>
        <div id="${timerId}" style="font-family:'Inter',sans-serif;font-weight:800;font-size:13px;color:rgba(255,255,255,.92);letter-spacing:.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${formatCountdown(a.ends_at)}</div>
      </div>`:''}
    </div>
    <div class="shirt-card-body">
      <div class="shirt-card-club">${escapeHtml(a.club||'—')}</div>
      <div class="shirt-card-season">${escapeHtml([a.season,a.kind,a.size].filter(Boolean).join(' · '))}</div>
      ${a.personalization?`<div style="font-size:10px;color:var(--t2);margin-bottom:4px;font-family:'JetBrains Mono',monospace">${escapeHtml(a.personalization)}</div>`:''}
      <div class="fx-b mb8">
        <div>
      ...
      ${a.highest_bidder_nick?`<div style="font-size:9px;font-family:'JetBrains Mono',monospace;color:var(--t2);margin-bottom:8px"><span style="display:inline-flex;align-items:center;gap:5px">${icon('trophy',10)}${escapeHtml(a.highest_bidder_nick)}</span></div>`:''}
      ${a.winner_nick&&isEnded?`<div style="font-size:9px;font-family:'JetBrains Mono',monospace;color:var(--gold);margin-bottom:8px"><span style="display:inline-flex;align-items:center;gap:5px;color:var(--gold)">${icon('trophy',10)}${db.lang==='pl'?'Wygrał':'Won by'}: ${escapeHtml(a.winner_nick)}</span></div>`:''}
      ${a.status==='cancelled'&&a.cancel_reason?`<div style="font-size:10px;color:var(--red);margin-bottom:8px;line-height:1.4"><b>${db.lang==='pl'?'Powód':'Reason'}:</b> ${escapeHtml(a.cancel_reason)}</div>`:''}
      <div class="div" style="margin:6px 0"></div>
      <div class="fx-c gap6" style="flex-wrap:wrap">
        ...
        <button class="btn btn-g" style="font-size:10px;padding:6px;flex-shrink:0" onclick="openAuctionComments('${a.id}')">${icon('messageCircle',12,"display:inline-block;vertical-align:-2px;margin-right:4px")}${db.lang==='pl'?'Pytania':'Questions'}</button>
        ${isWinner?`<button class="btn" style="font-size:10px;padding:6px;flex-shrink:0;background:rgba(var(--gold-rgb),.15);border:1px solid rgba(var(--gold-rgb),.3);color:var(--gold)" onclick="contactSeller('${a.user_id}','${escJsAttr(a.seller_nick||'')}')">${icon('messageCircle',12,"display:inline-block;vertical-align:-2px;margin-right:5px")}${db.lang==='pl'?'Kontakt':'Contact'}</button>`:''}
        ${isOwn&&a.status==='sold'&&a.winner_id?`<button class="btn" style="font-size:10px;padding:6px;flex-shrink:0;background:rgba(var(--gold-rgb),.15);border:1px solid rgba(var(--gold-rgb),.3);color:var(--gold)" onclick="contactSeller('${a.winner_id}','${escJsAttr(a.winner_nick||'')}')">${icon('messageCircle',12,"display:inline-block;vertical-align:-2px;margin-right:5px")}${db.lang==='pl'?'Napisz do zwycięzcy':'Message winner'}</button>`:''}
        ${adminMode?`<button class="btn btn-d" style="font-size:10px;padding:6px;flex-shrink:0" onclick="adminRemoveAuction('${a.id}')">${icon('trash',12,"display:inline-block;vertical-align:-2px;margin-right:4px")}${db.lang==='pl'?'Usuń (admin)':'Delete (admin)'}</button>`:''}
      </div>
      <div class="fx-b" style="margin-top:8px;padding-top:6px;border-top:1px solid rgba(239,68,68,.06)">
        <div class="mkt-seller fx-c gap6" style="color:rgba(239,68,68,.7)">
          <div style="width:18px;height:18px;border-radius:50%;background:linear-gradient(135deg,#ef4444,#7f1d1d);display:flex;align-items:center;justify-content:center;font-size:9px;color:#fff;font-family:'Inter',sans-serif;font-weight:800">${escapeHtml((a.seller_nick||'?')[0].toUpperCase())}</div>
          ${escapeHtml(a.seller_nick||'—')}
        </div>
...
async function openBidHistory(auctionId){
  ...
  <div class="vh-row">
    <div class="fx-c gap8" style="gap:8px">
      ...
      <span style="font-size:12px;font-weight:600">${escapeHtml(b.bidder_nick||'—')}</span>
    </div>
...
async function loadAuctionComments(auctionId){
  const{data,error}=await sb.from('auction_comments').select('*').eq('auction_id',auctionId).order('created_at',{ascending:true}).limit(100);
  if(error){console.error('loadAuctionComments error:',error);return[];}
  return data||[];
}
async function openAuctionComments(auctionId){
  document.getElementById('modal-title').textContent=db.lang==='pl'?'Pytania i odpowiedzi':'Questions & Answers';
  document.getElementById('modal-body').innerHTML=`
    <div id="ac-list" class="empty-state"><div class="empty-icon loading-pulse">${icon('messageCircle',28)}</div></div>
    <div class="div" style="margin:14px 0"></div>
    ${currentUser?`
    <div class="form-group" style="margin-bottom:10px">
      <textarea class="form-input" id="ac-input" rows="2" placeholder="${db.lang==='pl'?'Zadaj pytanie sprzedającemu — np. o wady, rozmiar, wysyłkę...':'Ask the seller a question — e.g. about defects, size, shipping...'}" maxlength="500"></textarea>
    </div>
    <div class="fx-c gap8" style="justify-content:flex-end">
      <button class="btn btn-g" onclick="closeModal()">${db.lang==='pl'?'Zamknij':'Close'}</button>
      <button class="btn btn-p" onclick="postAuctionComment('${auctionId}')">${db.lang==='pl'?'Wyślij':'Send'}</button>
    </div>`:`
    <div class="fx-c gap8" style="justify-content:flex-end">
      <button class="btn btn-g" onclick="closeModal()">${db.lang==='pl'?'Zamknij':'Close'}</button>
    </div>`}`;
  document.getElementById('modal-overlay').classList.add('open');
  await renderAuctionComments(auctionId);
}
async function renderAuctionComments(auctionId){
  const list=document.getElementById('ac-list');
  if(!list)return;
  const comments=await loadAuctionComments(auctionId);
  if(!comments.length){list.className='empty-state';list.innerHTML=`<div class="empty-icon">${icon('messageCircle',36)}</div><div class="empty-text">${db.lang==='pl'?'BRAK PYTAŃ — BĄDŹ PIERWSZY':'NO QUESTIONS YET — BE THE FIRST'}</div>`;return;}
  list.className='';
  list.innerHTML=comments.map(c=>{
    const canDelete=currentUser&&(currentUser.id===c.user_id||adminMode);
    return `
    <div class="vh-row" style="display:block;padding:10px 0">
      <div class="fx-b" style="margin-bottom:3px">
        <span style="font-size:11px;font-weight:700;color:var(--t1)">${escapeHtml(c.nick||'—')}</span>
        <span class="fx-c gap6">
          <span style="font-size:8px;font-family:'JetBrains Mono',monospace;color:var(--t3)">${c.created_at?new Date(c.created_at).toLocaleString(db.lang==='pl'?'pl-PL':'en-GB'):''}</span>
          ${canDelete?`<button onclick="deleteAuctionComment('${c.id}','${auctionId}')" style="background:none;border:none;cursor:pointer;color:var(--t3);padding:0;display:flex" title="${db.lang==='pl'?'Usuń':'Delete'}">${icon('trash',11)}</button>`:''}
        </span>
      </div>
...
async function deleteAuctionComment(commentId,auctionId){
  if(!await customConfirm(db.lang==='pl'?'Usunąć to pytanie?':'Delete this question?'))return;
  const{error}=await sb.from('auction_comments').delete().eq('id',commentId);
  if(error){toast((db.lang==='pl'?'Błąd: ':'Error: ')+error.message);return;}
  toast(db.lang==='pl'?'Usunięto':'Deleted');
  renderAuctionComments(auctionId);
}
async function postAuctionComment(auctionId){
  if(!currentUser){toast(db.lang==='pl'?'Musisz być zalogowany':'You must be logged in');return;}
  const input=document.getElementById('ac-input');
  const content=(input?.value||'').trim();
  if(!content){toast(db.lang==='pl'?'Wpisz treść pytania':'Enter your question');return;}
  const nick=db.profile?.nick||currentUser.email;
  const{error}=await sb.from('auction_comments').insert({auction_id:auctionId,user_id:currentUser.id,nick,content});
  if(error){toast(db.lang==='pl'?'Błąd wysyłania':'Error sending');return;}
  if(input)input.value='';
  await renderAuctionComments(auctionId);
  // Powiadom sprzedającego o nowym pytaniu
  const{data:auction}=await sb.from('auctions').select('user_id,club,season').eq('id',auctionId).maybeSingle();
  if(auction&&auction.user_id!==currentUser.id){
    await createNotification(auction.user_id,'question',db.lang==='pl'?'Nowe pytanie o licytację':'New question about your auction',db.lang==='pl'?`${nick} pyta o ${auction.club} ${auction.season||''}`:`${nick} asked about ${auction.club} ${auction.season||''}`,`auction:${auctionId}`);
  }
}

async function adminClearEndedAuctions(){
  if(!adminMode)return;
  const{data,error:fetchErr}=await sb.from('auctions').select('id').neq('status','active');
  if(fetchErr){toast((db.lang==='pl'?'Błąd: ':'Error: ')+fetchErr.message);return;}
  const count=(data||[]).length;
  if(!count){toast(db.lang==='pl'?'Brak zakończonych licytacji do usunięcia':'No ended auctions to clear');return;}
  if(!await customConfirm(db.lang==='pl'?`Usunąć ${count} zakończonych licytacji na stałe? Tej operacji nie można cofnąć.`:`Permanently delete ${count} ended auctions? This cannot be undone.`))return;
  const ids=data.map(a=>a.id);
  const{error}=await sb.from('auctions').delete().in('id',ids);
  if(error){toast((db.lang==='pl'?'Błąd: ':'Error: ')+error.message);return;}
  toast(db.lang==='pl'?`Usunięto ${count} zakończonych licytacji`:`Deleted ${count} ended auctions`);
  if(isOpen('auctions'))initAuctionsView();
}
async function adminRemoveAuction(auctionId){
  if(!adminMode)return;
  const reason=await customPrompt(db.lang==='pl'?'Powód usunięcia (widoczny dla sprzedającego i licytujących):':'Reason for removal (visible to seller and bidders):');
  if(reason===null)return;
  if(!await customConfirm(db.lang==='pl'?'Usunąć tę licytację jako moderator? Tej operacji nie można cofnąć.':'Delete this auction as moderator? This cannot be undone.'))return;
  ...
  const notifyIds=new Set((bidders||[]).map(b=>b.user_id));
  if(auction?.user_id)notifyIds.add(auction.user_id);
  const{error}=await sb.from('auctions').delete().eq('id',auctionId);
  if(error){toast((db.lang==='pl'?'Błąd: ':'Error: ')+error.message);return;}
  for(const uid of notifyIds){
    if(uid===currentUser.id)continue;
    await createNotification(uid,'cancelled',db.lang==='pl'?'Licytacja usunięta przez moderatora':'Auction removed by moderator',reason?(db.lang==='pl'?`Powód: ${reason}`:`Reason: ${reason}`):'','auctions');
  }
  toast(db.lang==='pl'?'Usunięto licytację':'Auction removed');
  closeModal();
  if(isOpen('auctions'))initAuctionsView();
}
async function cancelAuction(auctionId){
  ...
  let reason=await customPrompt(db.lang==='pl'?'Podaj powód anulowania (widoczny dla licytujących):':'Enter a reason for cancelling (visible to bidders):');
  if(reason===null)return;
  reason=reason.trim();
  if(!reason){toast(db.lang==='pl'?'Podanie powodu jest wymagane':'A reason is required');return;}
  const{error:updateError}=await sb.from('auctions').update({status:'cancelled',cancel_reason:reason}).eq('id',auctionId).eq('user_id',currentUser.id);
  if(updateError){
    console.error('cancelAuction update error:',updateError);
    toast((db.lang==='pl'?'Błąd anulowania: ':'Cancel error: ')+updateError.message);
    return;
  }
  ...
  if(hasBids){
    ...
    for(const bidderId of uniqueBidders){
      if(bidderId!==currentUser.id){
        await createNotification(bidderId,'cancelled',db.lang==='pl'?'Licytacja anulowana':'Auction cancelled',db.lang==='pl'?`${auction.club} ${auction.season||''} — sprzedający anulował licytację. Powód: ${reason}`:`${auction.club} ${auction.season||''} — seller cancelled the auction. Reason: ${reason}`,`auction:${auctionId}`);
      }
    }
  }
...
function openCreateAuction(){
  ...
  <div class="form-group">
    ...
    <select class="form-select" id="auc-shirt">
      ...
      ${myShirts.map(s=>`<option value="${s.id}">${escapeHtml(s.club||'?')} ${escapeHtml(s.season||'')} ${escapeHtml(kindLabel(s.kind)||'')}</option>`).join('')}
    </select>
  </div>
  <div class="g2">
    <div>
