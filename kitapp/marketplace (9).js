    ...
    <div class="ld-layout">
      ...
      <div>
        <div class="ld-price-row">
          ...
          ${l.legit_status==='approved'?legitStatusBadge('approved'):''}
        </div>
        <div class="ld-title">${escapeHtml(l.club||'—')}</div>
        <div class="ld-subtitle">${escapeHtml([l.season,l.kind].filter(Boolean).join(' · '))}</div>

        <div class="ld-tags-row">
          ${l.size?`<span class="ld-tag">${db.lang==='pl'?'Rozmiar':'Size'} <b>${escapeHtml(l.size)}</b></span>`:''}
          ${l.brand?`<span class="ld-tag">${escapeHtml(l.brand)}</span>`:''}
          ${l.condition?`<span class="ld-tag ld-tag-cond" style="color:${condColor(l.condition)}">${db.lang==='pl'?'Stan':'Cond.'} ${l.condition}/10</span>`:''}
