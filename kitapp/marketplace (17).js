    ...
    <div class="ld-layout">
      <div class="ld-gallery-row">
        ...
        <div class="ld-main-photo" id="ld-main-photo">
          ${photos.length?`<img src="${escapeHtml(photos[0])}" id="ld-main-img" alt="" onclick="ldZoom()">`:`<div class="shirt-photo-placeholder" style="height:100%;display:flex;align-items:center;justify-content:center">${icon('shirt',52)}</div>`}
          ...
          ${photos.length?`<button class="ld-zoom-btn" onclick="ldZoom()">${icon('maximize',12)} ${db.lang==='pl'?'Powiększ':'Zoom'}</button>`:''}
        </div>
      </div>
      <div>
...
async function initCommunityView(){
  ...
  grid.innerHTML=profiles.map(p=>{
    ...
    return `<div class="card" style="cursor:pointer;padding:0;overflow:hidden" onclick="viewProfile('${escJsAttr(p.nick||'')}')">
