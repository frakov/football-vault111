...
async function toggleUserBan(nick,userId){
  ...
  if(willBan){
    ...
    await sb.from('market_listings').delete().eq('user_id',userId);
    await sb.from('auctions').delete().eq('user_id',userId);
    await sb.from('profiles').update({visible:false}).eq('user_id',userId);
  }
  await createNotification(userId,willBan?'cancelled':'verified',willBan?(db.lang==='pl'?'Twoje konto zostało zawieszone':'Your account has been suspended'):(db.lang==='pl'?'Zawieszenie konta zostało cofnięte':'Your account suspension has been lifted'),willBan?(db.lang==='pl'?`Powód: ${reason}. Twoje ogłoszenia i licytacje zostały usunięte.`:`Reason: ${reason}. Your listings and auctions have been removed.`):(db.lang==='pl'?'Możesz ponownie normalnie korzystać z platformy.':'You can use the platform normally again.'),'profile');
  toast(willBan?(db.lang==='pl'?'Użytkownik zbanowany, jego ogłoszenia i licytacje usunięte':'User banned, their listings and auctions removed'):(db.lang==='pl'?'Ban cofnięty':'Ban lifted'));
  viewProfile(nick);
}
