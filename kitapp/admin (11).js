...
async function initLandingListings(){
  const grid=document.getElementById('lp-listings-grid');
  if(!grid)return;
  const{data,error}=await sb.from('market_listings').select('*').eq('mod_status','approved').order('listed_at',{ascending:false}).limit(8);
  if(error||!data||!data.length){
    grid.innerHTML=`<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon">${icon('cart',40)}</div><div class="empty-text">${db.lang==='pl'?'BRAK OGŁOSZEŃ — BĄDŹ PIERWSZY':'NO LISTINGS YET — BE THE FIRST'}</div></div>`;
    return;
  }
  grid.innerHTML=data.map(i=>{
    const price=parseFloat(i.price_pln||0);
    return `<div class="mkt-card" onclick="openAuthModal('register')" style="cursor:pointer">
      <div class="shirt-photo" style="background:var(--bg3)">
        ${i.photo?`<img src="${escapeHtml(i.photo)}" alt="" style="width:100%;height:100%;object-fit:cover">`:`<div class="shirt-photo-placeholder">${icon('shirt',36)}</div>`}
        ${i.legit_status==='approved'?`<div style="position:absolute;top:8px;left:8px">${legitStatusBadge('approved')}</div>`:''}
      </div>
      <div class="shirt-card-body">
        <div class="shirt-card-club">${escapeHtml(i.club||'—')}</div>
        <div class="shirt-card-season">${escapeHtml([i.season,i.kind,i.size].filter(Boolean).join(' · '))}</div>
        <div class="shirt-card-price" style="color:#14213d;margin-top:4px">${price?price.toFixed(2)+' PLN':'—'}</div>
      </div>
    </div>`;
  }).join('');
}
...
function initLandingSeoFooter(){
  ...
  if(!el)return;
  el.innerHTML=clubs.map((c,i)=>`<span onclick="openAuthModal('register')" style="white-space:nowrap;display:inline-block">${c}</span>${i<clubs.length-1?'<span class="lp-footer-seo-sep">·</span>':''}`).join('');
}
...
async function adminDeleteLegitCheck(reqId){
  if(!adminMode)return;
  if(!await customConfirm(db.lang==='pl'?'Usunąć to zgłoszenie na stałe?':'Delete this submission permanently?'))return;
  const{error}=await sb.from('legit_checks').delete().eq('id',reqId);
  if(error){toast((db.lang==='pl'?'Błąd: ':'Error: ')+error.message);return;}
  toast(db.lang==='pl'?'Usunięto zgłoszenie':'Submission deleted');
  const activeTab=document.getElementById('lc-tab-all')?.classList.contains('active')?'all':'pending';
  if(typeof renderLcAdminList==='function')renderLcAdminList(activeTab);
  else openLegitChecksPanel();
}
