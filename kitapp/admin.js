...
function showAdminBar(){
  ...
  bar.innerHTML=`<span>Tryb edycji strony</span>
    <span style="display:flex;align-items:center;gap:6px;background:rgba(244,244,239,.1);border-radius:4px;padding:3px 8px 3px 10px">
      ...
      <input type="text" id="admin-font-input" placeholder="np. Open Sans" value="${escapeHtml(db.siteFont||'')}" style="background:transparent;border:none;border-bottom:1px solid rgba(244,244,239,.3);color:#fff;font-size:11px;padding:2px 4px;width:110px;outline:none">
...
async function applySiteFont(){
  ...
  const {error}=await sb.from('site_content').upsert({key:'site.font',value:fontName},{onConflict:'key'});
  if(error){toast((db.lang==='pl'?'Nie zapisano na stałe: ':'Not saved permanently: ')+error.message);return;}
  toast(db.lang==='pl'?`Czcionka zmieniona na "${fontName}" — widoczna dla wszystkich`:`Font changed to "${fontName}" — visible to everyone`);
}
