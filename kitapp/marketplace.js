  ...
  document.getElementById('modal-body').innerHTML=`
    ...
    <div class="c8" style="grid-column:span 6">
      <div class="shirt-photo" style="border-radius:var(--r2);aspect-ratio:1;position:relative">
        ${l.photo?`<img src="${escapeHtml(l.photo)}" alt="" style="width:100%;height:100%;object-fit:contain;border-radius:var(--r2)">`:`<div class="shirt-photo-placeholder"><svg viewBox="0 0 24 24" width="52" height="52" fill="none" stroke="currentColor" stroke-width="1.1" stroke-linecap="round" stroke-linejoin="round"><path d="M20.38 3.46L16 2a4 4 0 01-8 0L3.62 3.46a2 2 0 00-1.34 2.23l.58 3.57a1 1 0 00.99.84H6v10c0 1.1.9 2 2 2h8a2 2 0 002-2V10h2.15a1 1 0 00.99-.84l.58-3.57a2 2 0 00-1.34-2.23z"/></svg></div>`}
      </div>
    </div>
    <div class="c4" style="grid-column:span 6">
      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--t2);letter-spacing:.04em;margin-bottom:8px">${escapeHtml([l.season,l.kind,l.size].filter(Boolean).join(' · '))}</div>
      ...
      ${l.condition?`<div style="font-size:11px;color:${condColor(l.condition)};font-family:'JetBrains Mono',monospace;margin-bottom:14px">${db.lang==='pl'?'Stan':'Condition'}: ${escapeHtml(String(l.condition))}/10</div>`:'<div style="margin-bottom:14px"></div>'}
      ${!isOwn?`
      <button class="btn btn-mkt w100 mb8" onclick="contactSeller('${l.user_id||''}','${escJsAttr(l.seller_nick||'')}')">${icon('messageCircle',13,"display:inline-block;vertical-align:-2px;margin-right:6px")}${T('mkt_contact')}</button>
      ...
      <div class="mkt-seller fx-c gap8" style="cursor:pointer;margin-bottom:10px" onclick="closeModal();viewProfile('${escJsAttr(l.seller_nick||'')}')">
        <div style="width:28px;height:28px;border-radius:50%;background:linear-gradient(135deg,var(--a) 0%,color-mix(in srgb,var(--a) 50%,#000) 100%);display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;color:#fff;font-family:'Inter',sans-serif;font-weight:800">${escapeHtml((l.seller_nick||'?')[0].toUpperCase())}</div>
        <span style="font-size:12px;font-weight:600;color:var(--t1)">${escapeHtml(l.seller_nick||'—')}</span>${verifiedBadgeHtml(l.seller_nick,14)}
      </div>
      ...
      ${l.notes?`<div style="font-size:12px;color:var(--t2);line-height:1.6;margin-top:14px;padding-top:14px;border-top:1px solid var(--border)">${escapeHtml(l.notes)}</div>`:''}
      <div class="div" style="margin:14px 0"></div>
...
function openProfileView(p){
  ...
  <div class="fx-c gap16 mb18">
    ...
    <div><div style="font-family:'Inter',sans-serif;font-weight:800;font-size:24px;letter-spacing:.06em">${escapeHtml(p.nick||'—')}${p.is_verified?`<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#2563eb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:2px;margin-left:6px"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>`:''}</div>
