  ...
  grid.innerHTML=profiles.map(p=>{
    ...
    return `<div class="card" style="cursor:pointer;padding:0;overflow:hidden" onclick="viewProfile('${escJsAttr(p.nick||'')}')">
      ...
      <div style="padding:14px">
        <div class="fx-c gap10 mb8">
          <div class="profile-avatar" style="width:36px;height:36px;font-size:15px">${nick[0].toUpperCase()}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:800;font-size:14px;letter-spacing:.03em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${nick}${p.is_verified?verifiedBadgeHtml(p.nick,13):''}</div>
            <div style="font-size:9px;font-family:'JetBrains Mono',monospace;color:var(--t3)">${p.shirt_count||0} ${db.lang==='pl'?'koszulek':'jerseys'}</div>
          </div>
        </div>
