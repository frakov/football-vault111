...
function openProfileView(p){
  ...
  <div class="fx-c gap16 mb18">
    ...
    <div><div style="font-family:'Inter',sans-serif;font-weight:800;font-size:24px;letter-spacing:.06em">${escapeHtml(p.nick||'—')}${p.is_verified?`<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="#2563eb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block;vertical-align:2px;margin-left:6px"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>`:''}</div>
      ...
      ${adminMode?`<button class="btn ${p.is_verified?'btn-d':'btn-p'}" style="padding:5px 12px;font-size:10px;margin-top:10px" onclick="toggleUserVerified('${escJsAttr(p.nick||'')}','${p.user_id}')">${p.is_verified?(db.lang==='pl'?'Cofnij weryfikację':'Remove verification'):(db.lang==='pl'?'Zweryfikuj użytkownika':'Verify user')}</button>`:''}
    </div>
  </div>
  ${shirts.length?`
