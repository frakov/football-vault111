...
async function sbListOnMarket(shirt){
  ...
  const{data:prof}=await sb.from('profiles').select('is_verified').eq('user_id',currentUser.id).maybeSingle();
  ...
  const {error}=await sb.from('market_listings').upsert({
    ...
    club:shirt.club, season:shirt.season, brand:shirt.brand, kind:kindLabel(shirt.kind), size:shirt.customSize||shirt.size,
    condition:shirt.condition, personalization:shirt.personalization,
    price_pln:toPLN(shirt.listPrice||shirt.marketValue||shirt.buyPrice), notes:shirt.notes,
    photo:shirt.photos&&shirt.photos[0]?shirt.photos[0]:null, photos:shirt.photos||[],
    accepts_offers:shirt.acceptsOffers!==false, min_offer:shirt.minOffer||null, legit_status:shirt.legitStatus||null,
    mod_status:isVerified?'approved':'pending', mod_reason:null,
...
function bMarket(){
  ...
  <div id="mkt-section-listings">
    <div class="mkt-mobile-bar">
      <div class="field field-search" style="padding:8px 12px;flex:1">${icon('search',13)}<input type="text" id="mkt-search-mobile" placeholder="${db.lang==='pl'?'Klub, sezon, nick...':'Club, season, nick...'}" oninput="filterMarket()" style="font-size:12px"></div>
      <button class="mkt-filters-btn" id="mkt-filters-btn" onclick="toggleMktFiltersSheet()">${icon('sliders',14)}<span id="mkt-filters-count" style="display:none"></span></button>
    </div>
    <div id="mkt-filters-scrim" onclick="toggleMktFiltersSheet()"></div>
    <div class="mkt-layout">
      <div class="mkt-sidebar" id="mkt-sidebar">
        <div class="mkt-sidebar-mobile-head">
          <span>${db.lang==='pl'?'Filtry':'Filters'}</span>
          <button onclick="toggleMktFiltersSheet()">${icon('x',16)}</button>
        </div>
        ...
        <div class="mkt-sidebar-mobile-apply">
          <button class="btn btn-mkt w100" onclick="toggleMktFiltersSheet()">${db.lang==='pl'?'Pokaż wyniki':'Show results'} <span id="mkt-count-mobile"></span></button>
        </div>
      </div>
      <div>
        <div class="fx-b mb12">
...
function filterMarket(){
  const qDesktop=document.getElementById('mkt-search')?.value||'';
  const qMobile=document.getElementById('mkt-search-mobile')?.value||'';
  const q=(document.activeElement?.id==='mkt-search-mobile'?qMobile:(qDesktop||qMobile));
  // synchronizuj oba pola wyszukiwania (desktop i mobilny pasek), żeby zachowywały się jak jedno
  const searchEl=document.getElementById('mkt-search'),searchElMobile=document.getElementById('mkt-search-mobile');
  if(searchEl&&searchEl.value!==q)searchEl.value=q;
  if(searchElMobile&&searchElMobile.value!==q)searchElMobile.value=q;
  _mktFilters.q=q.toLowerCase();
  ...
  const countText=`${items.length} ${db.lang==='pl'?'dostępnych ogłoszeń':'listings available'}`;
  const cEl=document.getElementById('mkt-count');
  if(cEl)cEl.textContent=countText;
  const cElMobile=document.getElementById('mkt-count-mobile');
  if(cElMobile)cElMobile.textContent=`(${items.length})`;
  const activeCount=[f.club,f.kind,f.size,f.priceMin!=null?'x':'',f.priceMax!=null?'x':''].filter(Boolean).length;
  const badge=document.getElementById('mkt-filters-count');
  if(badge){badge.textContent=activeCount>0?activeCount:'';badge.style.display=activeCount>0?'flex':'none';}
  const btn=document.getElementById('mkt-filters-btn');
  if(btn)btn.classList.toggle('has-active',activeCount>0);
}
function toggleMktFiltersSheet(){
  const sidebar=document.getElementById('mkt-sidebar');
  const scrim=document.getElementById('mkt-filters-scrim');
  if(!sidebar)return;
...
function clearMktFilters(){
  ...
  const sMobile=document.getElementById('mkt-search-mobile');if(sMobile)sMobile.value='';
...
function buildMktSidebarClubs(items){
  ...
  el.innerHTML=clubs.map(c=>`<div class="mkt-club-row ${_mktFilters.club===c?'active':''}" data-club="${escapeHtml(c)}" onclick="setMktClub('${escJsAttr(c)}')"><span>${escapeHtml(c)}</span><span>${counts[c]}</span></div>`).join('');
}
...
function renderPublicWishlistCards(items){
  ...
  return items.map(w=>{
    const nick=escapeHtml(w._nick||'?');
    const isOwn=w.user_id===currentUser?.id;
    return `<div class="card" style="border-color:rgba(20,33,61,.15)">
      <div class="fx-b mb8">
        <div style="font-family:'Inter',sans-serif;font-weight:800;font-size:18px;color:var(--t1)">${escapeHtml(w.club||(db.lang==='pl'?'Dowolny klub':'Any club'))}</div>
      ...
      <div style="display:flex;flex-direction:column;gap:4px;margin-bottom:10px">
        ...
        ${w.size?`<div style="font-size:10px;font-family:'JetBrains Mono',monospace;color:var(--t2)">${icon('ruler',11,"display:inline-block;vertical-align:-2px;margin-right:4px")}${escapeHtml(w.size)}</div>`:''}
      ...
      <div class="fx-b">
        <div class="fx-c gap6" style="cursor:pointer" onclick="viewProfile('${escJsAttr(w._nick||'')}')">
        ...
        ${!isOwn?`<button class="btn btn-g" style="padding:4px 10px;font-size:10px" onclick="contactSeller('${w.user_id}','${escJsAttr(w._nick||'')}')">${icon('messageCircle',12,"display:inline-block;vertical-align:-2px;margin-right:5px")}Napisz</button>`:`<span style="font-size:9px;font-family:'JetBrains Mono',monospace;color:var(--t3)">TWOJA</span>`}
      </div>
    </div>`;
  }).join('');
}
...
async function loadPublicProfilesSection(){
  ...
  el.innerHTML=`<div class="g3">${profiles.map(p=>`
    <div class="card fx-c gap12" style="cursor:pointer" onclick="viewProfile('${escJsAttr(p.nick||'')}')">
      <div class="profile-avatar" style="width:44px;height:44px;font-size:18px">${escapeHtml((p.nick||'?')[0].toUpperCase())}</div>
      <div style="flex:1;min-width:0">
        <div style="font-family:'Inter',sans-serif;font-weight:800;font-size:16px;letter-spacing:.04em">${escapeHtml(p.nick||'—')}</div>
        ${p.bio?`<div style="font-size:10px;color:var(--t2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${escapeHtml(p.bio)}</div>`:''}
      ...
      <button class="btn btn-g" style="padding:4px 10px;font-size:10px;flex-shrink:0" onclick="event.stopPropagation();contactSeller('${p.user_id}','${escJsAttr(p.nick||'')}')">${icon('messageCircle',13)}</button>
    </div>`).join('')}</div>`;
}
...
function mktCardHtml(i){
  ...
  const sellerNick=escapeHtml(i.seller_nick||'—');
  return `<div class="mkt-card" onclick="openListingDetail('${i.id}')">
      <div class="shirt-photo" style="background:var(--bg3)">
        ${i.photo?`<img src="${escapeHtml(i.photo)}" alt="" style="width:100%;height:100%;object-fit:cover">`:`<div class="shirt-photo-placeholder"><svg viewBox="0 0 24 24" width="40" height="40" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.38 3.46L16 2a4 4 0 01-8 0L3.62 3.46a2 2 0 00-1.34 2.23l.58 3.57a1 1 0 00.99.84H6v10c0 1.1.9 2 2 2h8a2 2 0 002-2V10h2.15a1 1 0 00.99-.84l.58-3.57a2 2 0 00-1.34-2.23z"/></svg></div>`}
        <div style="position:absolute;top:8px;right:8px"><span class="sh-status public">Marketplace</span></div>
        ${isOwn?`<div style="position:absolute;top:8px;left:8px;background:${i.mod_status==='pending'?'rgba(245,158,11,.9)':i.mod_status==='rejected'?'rgba(192,57,43,.9)':'rgba(var(--a-rgb),.8)'};border-radius:6px;padding:2px 8px;font-family:'JetBrains Mono',monospace;font-size:8px;color:#fff">${i.mod_status==='pending'?(db.lang==='pl'?'OCZEKUJE NA AKCEPTACJĘ':'PENDING REVIEW'):i.mod_status==='rejected'?(db.lang==='pl'?'ODRZUCONE':'REJECTED'):'TWOJE'}</div>`:isMktRecent(i.listed_at)?`<div class="mkt-badge">${db.lang==='pl'?'NOWE':'NEW'}</div>`:''}
        <div style="position:absolute;bottom:8px;right:8px">${likeButtonHtml(i.id)}</div>
      </div>
      <div class="shirt-card-body">
        <div class="shirt-card-club">${escapeHtml(i.club||'—')}${i.legit_status==='approved'?` ${legitStatusBadge('approved')}`:''}</div>
        <div class="shirt-card-season">${escapeHtml([i.season,i.kind,i.size].filter(Boolean).join(' · '))}</div>
        ${i.personalization?`<div style="font-size:10px;color:var(--t2);margin-bottom:4px;font-family:'JetBrains Mono',monospace">${escapeHtml(i.personalization)}</div>`:''}
        <div class="fx-b mb8">
          <div class="shirt-card-price" style="color:#14213d">${mv?mv.toFixed(2)+' PLN':'—'}</div>
          ${i.condition?`<div style="font-size:9px;font-family:'JetBrains Mono',monospace;color:${condColor(i.condition)}">${escapeHtml(String(i.condition))}/10</div>`:''}
        </div>
        <div class="div" style="margin:8px 0"></div>
        <div class="fx-b">
          <div class="mkt-seller fx-c gap6" style="cursor:pointer" onclick="event.stopPropagation();viewProfile('${escJsAttr(i.seller_nick||'')}')">
            <div style="width:22px;height:22px;border-radius:50%;background:linear-gradient(135deg,var(--a) 0%,color-mix(in srgb,var(--a) 50%,#000) 100%);display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;color:var(--on-a,#fff);font-family:'Inter',sans-serif;font-weight:800">${sellerNick[0].toUpperCase()}</div>
            ${sellerNick}${verifiedBadgeHtml(i.seller_nick)}
          </div>
          ${isOwn?`<button class="btn btn-d" style="padding:4px 10px;font-size:10px" onclick="event.stopPropagation();removeListing('${i.id}')">${icon('x',11,"display:inline-block;vertical-align:-1px;margin-right:4px")} ${T('mkt_remove')}</button>`:`<button class="btn btn-mkt" style="padding:4px 10px;font-size:10px" onclick="event.stopPropagation();contactSeller('${i.user_id||''}','${escJsAttr(i.seller_nick||'')}')">${icon('messageCircle',12,"display:inline-block;vertical-align:-2px;margin-right:5px")} ${T('mkt_contact')}</button>`}
        </div>
...
function verifiedBadgeHtml(nick,size){
  ...
  size=size||12;
  return `<svg viewBox="0 0 24 24" width="${size}" height="${size}" style="display:inline-block;vertical-align:-2px;flex-shrink:0" title="${db.lang==='pl'?'Zweryfikowany sprzedający':'Verified seller'}"><circle cx="12" cy="12" r="10" fill="#2563eb"/><path d="M7.5 12.5l3 3 6-6" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>`;
}
...
async function toggleUserBan(nick,userId){
  if(!adminMode)return;
  const {data}=await sb.from('profiles').select('is_banned').eq('user_id',userId).maybeSingle();
  const willBan=!(data?.is_banned);
  let reason=null;
  if(willBan){
    reason=await customPrompt(db.lang==='pl'?'Powód zawieszenia konta (widoczny dla użytkownika):':'Reason for suspending this account (visible to the user):');
    if(reason===null)return;
    reason=reason.trim();
    if(!reason){toast(db.lang==='pl'?'Podanie powodu jest wymagane':'A reason is required');return;}
  } else {
    if(!await customConfirm(db.lang==='pl'?'Odbanować tego użytkownika?':'Unban this user?'))return;
  }
  const {error}=await sb.from('profiles').update({is_banned:willBan,ban_reason:willBan?reason:null}).eq('user_id',userId);
  if(error){toast((db.lang==='pl'?'Błąd: ':'Error: ')+error.message);return;}
  if(willBan){
    // Ban usuwa też całą publiczną aktywność użytkownika — ogłoszenia, licytacje, widoczność w Społeczności.
    // Każdy krok sprawdzany osobno i zgłaszany — żadne ciche niepowodzenie.
    const cleanupErrors=[];
    const{error:visErr}=await sb.from('profiles').update({visible:false}).eq('user_id',userId);
    if(visErr)cleanupErrors.push('widoczność w Społeczności: '+visErr.message);
    const{error:listErr}=await sb.from('market_listings').delete().eq('user_id',userId);
    if(listErr)cleanupErrors.push('ogłoszenia: '+listErr.message);
    const{error:aucErr}=await sb.from('auctions').delete().eq('user_id',userId);
    if(aucErr)cleanupErrors.push('licytacje: '+aucErr.message);
    await createNotification(userId,'cancelled',db.lang==='pl'?'Twoje konto zostało zawieszone':'Your account has been suspended',db.lang==='pl'?`Powód: ${reason}. Twoje ogłoszenia i licytacje zostały usunięte.`:`Reason: ${reason}. Your listings and auctions have been removed.`,'profile');
    if(cleanupErrors.length){
      toast((db.lang==='pl'?'Ban zapisany, ale nie wszystko się wyczyściło — ':'Ban saved, but cleanup incomplete — ')+cleanupErrors.join('; '));
    } else {
      toast(db.lang==='pl'?'Użytkownik zbanowany, jego ogłoszenia i licytacje usunięte':'User banned, their listings and auctions removed');
    }
  } else {
    await createNotification(userId,'verified',db.lang==='pl'?'Zawieszenie konta zostało cofnięte':'Your account suspension has been lifted',db.lang==='pl'?'Możesz ponownie normalnie korzystać z platformy.':'You can use the platform normally again.','profile');
    toast(db.lang==='pl'?'Ban cofnięty':'Ban lifted');
  }
  viewProfile(nick);
}
...
async function openListingDetail(id){
  ...
  document.getElementById('modal-title').textContent=(db.lang==='pl'?'Marketplace':'Marketplace')+' › '+(l.club||(db.lang==='pl'?'Ogłoszenie':'Listing'));
  document.getElementById('modal-body').innerHTML=`<div class="empty-state"><div class="empty-icon loading-pulse">${icon('clock',28)}</div></div>`;
  document.getElementById('modal-box')?.classList.add('modal-fullscreen');
  document.getElementById('modal-overlay').classList.add('open');

  // Zdjęcia są zapisane bezpośrednio w ogłoszeniu (publicznie czytelne) — NIE dociągamy z prywatnej
  // tabeli kolekcji sprzedającego, bo kupujący nie ma do niej dostępu (i nie powinien mieć).
  let photos=(l.photos&&l.photos.length)?l.photos:[l.photo].filter(Boolean);
  window._ldPhotos=photos;
  window._ldIdx=0;

  const fullTitle=[l.club,l.kind?`${db.lang==='pl'?'Koszulka':'Jersey'} ${l.kind}`:null].filter(Boolean).join(db.lang==='pl'?' – ':' – ');
  const condBg=l.condition?ldCondBg(l.condition):null;

  document.getElementById('modal-body').innerHTML=`
    <div class="ld-layout">
      <div class="ld-gallery-row">
        ${photos.length>1?`<div class="ld-vthumbs" id="ld-thumbs">${photos.map((p,i)=>`<div class="ld-thumb ${i===0?'active':''}" onclick="ldSetPhoto(${i})"><img src="${escapeHtml(p)}" alt=""></div>`).join('')}</div>`:''}
        <div class="ld-main-photo" id="ld-main-photo">
          ${photos.length?`<img src="${escapeHtml(photos[0])}" id="ld-main-img" alt="">`:`<div class="ld-photo-empty">${icon('shirt',44)}<span>${db.lang==='pl'?'Brak zdjęcia':'No photo'}</span></div>`}
          <div style="position:absolute;top:12px;right:12px;z-index:2">${likeButtonHtml(l.id,'lg')}</div>
          ${photos.length>1?`<button class="ld-nav-btn ld-nav-prev" onclick="ldNavPhoto(-1)">‹</button><button class="ld-nav-btn ld-nav-next" onclick="ldNavPhoto(1)">›</button>`:''}
        </div>
      </div>
      <div>
        <div class="ld-title">${escapeHtml(fullTitle||'—')}</div>
        <div class="ld-meta-line">
          <span>${icon('clock',11)} ${db.lang==='pl'?'Opublikowano':'Posted'} ${agoLabel(l.listed_at)}</span>
          <span>·</span>
          <span>${icon('eye',11)} ${(l.views||0)+1} ${db.lang==='pl'?'wyświetleń':'views'}</span>
        </div>
        <div class="ld-price-big">${mv?mv.toFixed(2)+' PLN':'—'}</div>

        <div class="ld-seller-row" onclick="closeModal();viewProfile('${escJsAttr(l.seller_nick||'')}')">
          <div class="ld-seller-left">
            <div class="ld-avatar">${escapeHtml((l.seller_nick||'?')[0].toUpperCase())}</div>
            <div>
              <div class="ld-seller-name-row"><span style="font-size:14px;font-weight:700;color:var(--t1)">${escapeHtml(l.seller_nick||'—')}</span>${verifiedBadgeHtml(l.seller_nick,14)}</div>
              <div class="ld-seller-sub">${db.lang==='pl'?'Z KOLEKCJI SPRZEDAJĄCEGO':"FROM SELLER'S COLLECTION"}</div>
            </div>
          </div>
          <span class="ld-seller-link">${db.lang==='pl'?'Zobacz profil':'View profile'} →</span>
        </div>

        <div class="ld-specs-grid">
          ${l.season?`<div><span class="ld-spec-label">${db.lang==='pl'?'Sezon':'Season'}</span><span class="ld-spec-val">${escapeHtml(l.season)}</span></div>`:''}
          ${l.size?`<div><span class="ld-spec-label">${db.lang==='pl'?'Rozmiar':'Size'}</span><span class="ld-spec-val">${escapeHtml(l.size)}</span></div>`:''}
          ${l.brand?`<div><span class="ld-spec-label">${db.lang==='pl'?'Marka':'Brand'}</span><span class="ld-spec-val">${escapeHtml(l.brand)}</span></div>`:''}
          ${l.condition?`<div><span class="ld-spec-label">${db.lang==='pl'?'Stan':'Condition'}</span><span class="ld-spec-val" style="color:${condBg.fg}">${l.condition}/10</span></div>`:''}
          ${l.personalization?`<div style="grid-column:1/-1"><span class="ld-spec-label">${db.lang==='pl'?'Personalizacja':'Personalization'}</span><span class="ld-spec-val">${escapeHtml(l.personalization)}</span></div>`:''}
        </div>
        ${l.legit_status==='approved'?`<div style="margin-top:14px" title="${db.lang==='pl'?'Ogłoszenie przeszło weryfikację autentyczności KitBase':'This listing passed KitBase authenticity verification'}">${legitStatusBadge('approved')}</div>`:''}

        <div class="ld-desc"><div class="ld-section-label">${db.lang==='pl'?'Opis':'Description'}</div>${l.notes?escapeHtml(l.notes):`<span style="color:var(--t3);font-style:italic">${db.lang==='pl'?'Sprzedający nie dodał opisu.':'The seller did not add a description.'}</span>`}</div>

        ${!isOwn?`<button class="btn btn-mkt w100" style="margin-top:14px" onclick="contactSeller('${l.user_id||''}','${escJsAttr(l.seller_nick||'')}')">${icon('messageCircle',13,"display:inline-block;vertical-align:-2px;margin-right:6px")}${T('mkt_contact')}</button>`:''}
        ${!isOwn&&l.accepts_offers!==false?`<button class="btn btn-g w100" style="margin-top:8px" onclick="openMakeOffer('${l.id}')">${icon('tag',13,"display:inline-block;vertical-align:-2px;margin-right:6px")}${db.lang==='pl'?'Zaproponuj ofertę':'Make an offer'}${l.min_offer?` (min. ${parseFloat(l.min_offer).toFixed(0)} PLN)`:''}</button>`:''}
        ${!isOwn&&l.accepts_offers===false?`<div style="font-size:9.5px;color:var(--t3);font-family:'JetBrains Mono',monospace;letter-spacing:.06em;margin-top:10px;text-align:center">${db.lang==='pl'?'SPRZEDAJĄCY NIE AKCEPTUJE OFERT':'SELLER DOES NOT ACCEPT OFFERS'}</div>`:''}

        <div class="fx-c gap8" style="margin-top:10px;flex-wrap:wrap">
          <button class="btn btn-g" style="flex:1;font-size:10px;padding:6px" onclick="shareListing('${l.id}')">${db.lang==='pl'?'Udostępnij':'Share'}</button>
          ${!isOwn?`<button class="btn btn-g" style="flex:1;opacity:.75;font-size:10px;padding:6px" onclick="reportListing('${l.id}')">${flagIcon}${db.lang==='pl'?'Zgłoś':'Report'}</button>`:''}
        </div>
        ${isOwn?`<button class="btn btn-d w100" style="margin-top:8px" onclick="closeModal();removeListing('${l.id}')">${icon('trash',12,"display:inline-block;vertical-align:-2px;margin-right:5px")}${T('mkt_remove')}</button>`:''}
        ${adminMode?`<button class="btn btn-d w100" style="margin-top:8px" onclick="adminRemoveListing('${l.id}')">${icon('trash',12,"display:inline-block;vertical-align:-2px;margin-right:5px")}${db.lang==='pl'?'Usuń (admin)':'Delete (admin)'}</button>`:''}
      </div>
    </div>`;
...
function ldCondBg(c){const n=parseInt(c);if(n>=8)return{bg:'rgba(34,139,73,.12)',fg:'#1e7a44'};if(n>=5)return{bg:'rgba(217,144,15,.13)',fg:'#a5670a'};return{bg:'rgba(192,57,43,.12)',fg:'#c0392b'};}
function ldNavPhoto(dir){
  const n=window._ldPhotos.length;
  window._ldIdx=(window._ldIdx+dir+n)%n;
  ldSetPhoto(window._ldIdx);
}
function ldSetPhoto(idx){
  window._ldIdx=idx;
  const img=document.getElementById('ld-main-img');
  if(img)img.src=window._ldPhotos[idx];
  document.querySelectorAll('#ld-thumbs .ld-thumb').forEach((t,i)=>t.classList.toggle('active',i===idx));
}
function shareListing(id){
  const url=location.origin+location.pathname+'#listing-'+id;
  if(navigator.share){navigator.share({title:'KitBase',url}).catch(()=>{});}
  else if(navigator.clipboard){navigator.clipboard.writeText(url).then(()=>toast(db.lang==='pl'?'Link skopiowany':'Link copied'));}
}
function ldCondColor(c){const n=parseInt(c);if(!n)return'#fff';if(n>=8)return'#4ade80';if(n>=5)return'#fbbf24';return'#f87171';}
...
function openMakeOffer(id){
  ...
  document.getElementById('modal-body').innerHTML=`
    <div style="font-size:13px;color:var(--t2);margin-bottom:14px">${escapeHtml(l.club||'')} ${escapeHtml(l.season||'')} — ${db.lang==='pl'?'cena wyjściowa':'asking price'} <b style="color:#14213d">${parseFloat(l.price_pln||0).toFixed(2)} PLN</b></div>
...
async function adminRemoveListing(id){
  if(!adminMode)return;
  const reason=await customPrompt(db.lang==='pl'?'Powód usunięcia (widoczny dla sprzedającego):':'Reason for removal (visible to seller):');
  if(reason===null)return;
  if(!await customConfirm(db.lang==='pl'?'Usunąć to ogłoszenie jako moderator?':'Delete this listing as moderator?'))return;
  const{data:listing}=await sb.from('market_listings').select('user_id,club,season').eq('id',id).maybeSingle();
  const {error}=await sb.from('market_listings').delete().eq('id',id);
  if(error){toast('Błąd: '+error.message);return;}
  if(listing?.user_id&&listing.user_id!==currentUser.id){
    await createNotification(listing.user_id,'cancelled',db.lang==='pl'?'Ogłoszenie usunięte przez moderatora':'Listing removed by moderator',reason?(db.lang==='pl'?`${listing.club||''} ${listing.season||''} — Powód: ${reason}`:`${listing.club||''} ${listing.season||''} — Reason: ${reason}`):'','market');
  }
...
function openProfileView(p){
  ...
  document.getElementById('modal-body').innerHTML=`
  <div class="fx-b" style="align-items:flex-start">
  <div class="fx-c gap16 mb18">
    <div class="profile-avatar">${escapeHtml((p.nick||'?')[0].toUpperCase())}</div>
    <div><div style="font-family:'Inter',sans-serif;font-weight:800;font-size:24px;letter-spacing:.06em">${escapeHtml(p.nick||'—')}${p.is_verified?`<svg viewBox="0 0 24 24" width="18" height="18" style="display:inline-block;vertical-align:2px;margin-left:6px" title="${db.lang==='pl'?'Zweryfikowany sprzedający':'Verified seller'}"><circle cx="12" cy="12" r="10" fill="#2563eb"/><path d="M7.5 12.5l3 3 6-6" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg>`:''}</div>
      ${p.bio?`<div style="font-size:13px;color:var(--t2);margin-top:4px">${escapeHtml(p.bio)}</div>`:''}
      ...
      ${p.is_banned?`<div style="font-size:10px;color:var(--red);margin-top:6px;font-weight:700">${db.lang==='pl'?'KONTO ZAWIESZONE':'ACCOUNT SUSPENDED'}${p.ban_reason?` — ${escapeHtml(p.ban_reason)}`:''}</div>`:''}
      ${adminMode?`<div class="fx-c gap6" style="margin-top:10px;flex-wrap:wrap">
        <button class="btn ${p.is_verified?'btn-d':'btn-p'}" style="padding:5px 12px;font-size:10px" onclick="toggleUserVerified('${escJsAttr(p.nick||'')}','${p.user_id}')">${p.is_verified?(db.lang==='pl'?'Cofnij weryfikację':'Remove verification'):(db.lang==='pl'?'Zweryfikuj użytkownika':'Verify user')}</button>
        <button class="btn ${p.is_banned?'btn-p':'btn-d'}" style="padding:5px 12px;font-size:10px" onclick="toggleUserBan('${escJsAttr(p.nick||'')}','${p.user_id}')">${p.is_banned?(db.lang==='pl'?'Odbanuj':'Unban'):(db.lang==='pl'?'Zbanuj użytkownika':'Ban user')}</button>
      </div>`:''}
    </div>
  </div>
  ${p.user_id&&p.user_id!==currentUser?.id?`<button class="like-btn-lg" id="profile-like-btn" onclick="toggleProfileLike('${p.user_id}')" style="flex-shrink:0">${icon('heart',14)} …</button>`:''}
  </div>
  ${shirts.length?`
  ...
  <div class="g3" id="profile-shirt-grid">${shirts.map(s=>`
    <div class="mkt-card" style="background:rgba(20,33,61,.04);border-color:rgba(20,33,61,.12)">
      <div class="shirt-photo" style="position:relative">${s.photo?`<img src="${escapeHtml(s.photo)}" alt="" style="width:100%;height:100%;object-fit:cover">`:`<div class="shirt-photo-placeholder"><svg viewBox="0 0 24 24" width="40" height="40" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round"><path d="M20.38 3.46L16 2a4 4 0 01-8 0L3.62 3.46a2 2 0 00-1.34 2.23l.58 3.57a1 1 0 00.99.84H6v10c0 1.1.9 2 2 2h8a2 2 0 002-2V10h2.15a1 1 0 00.99-.84l.58-3.57a2 2 0 00-1.34-2.23z"/></svg></div>`}
      ...
      <div class="shirt-card-body">
        ...
        <div class="shirt-card-season">${escapeHtml([s.season,s.kind].filter(Boolean).join(' · '))}</div>
  ...
  ${p.user_id?`
  <div class="div" style="margin:18px 0"></div>
  <div class="clabel">${db.lang==='pl'?'Komentarze':'Comments'}</div>
  <div id="profile-comments-list" style="max-height:180px;overflow-y:auto;margin-bottom:12px"></div>
  ${currentUser?`<div class="fx-c gap8">
    <input class="form-input" type="text" id="profile-comment-input" placeholder="${db.lang==='pl'?'Napisz komentarz...':'Write a comment...'}" style="flex:1" maxlength="500">
    <button class="btn btn-p" onclick="postProfileComment('${p.user_id}')">${db.lang==='pl'?'Wyślij':'Send'}</button>
  </div>`:`<div style="font-size:11px;color:var(--t3)">${db.lang==='pl'?'Zaloguj się, żeby skomentować':'Log in to comment'}</div>`}
  `:''}
  <div class="div" style="margin:18px 0"></div>
  <div style="text-align:right"><button class="btn btn-g" onclick="closeModal()">${T('cancel')}</button></div>`;
  document.getElementById('modal-overlay').classList.add('open');
  if(p.user_id){refreshProfileLikeUI(p.user_id);renderProfileComments(p.user_id);}
...
// ══════════════════════════════════════
// SPOŁECZNOŚĆ — publiczne kolekcje, polubienia profili, komentarze
// ══════════════════════════════════════
function bCommunity(){
  return `
  <div class="fx-b mb18 s1">
    <div><div class="ph-sub"><span class="dot"></span><span>${T('community')}</span></div><div class="ph-title">${db.lang==='pl'?'SPOŁECZNOŚĆ':'COMMUNITY'}</div></div>
  </div>
  <p style="font-size:12.5px;color:var(--t2);margin-bottom:18px;max-width:560px">${db.lang==='pl'?'Kolekcjonerzy, którzy udostępnili swoją kolekcję publicznie. Polub, zajrzyj, napisz komentarz.':'Collectors who made their collection public. Like, browse, leave a comment.'}</p>
  <div class="g3" id="community-grid"><div class="empty-state"><div class="empty-icon loading-pulse">${icon('users',28)}</div></div></div>`;
}
async function initCommunityView(){
  const grid=document.getElementById('community-grid');
  if(!grid)return;
  const{data,error}=await sb.from('profiles').select('*').eq('visible',true);
  const profiles=(data||[]).filter(p=>p.user_id!==currentUser?.id);
  if(error||!profiles.length){grid.className='empty-state';grid.innerHTML=`<div class="empty-icon">${icon('users',44)}</div><div class="empty-text">${db.lang==='pl'?'NIKT JESZCZE NIE UDOSTĘPNIŁ KOLEKCJI':'NO ONE HAS SHARED A COLLECTION YET'}</div>`;return;}
  grid.className='g3';
  const likeCounts=await loadProfileLikeCounts(profiles.map(p=>p.user_id));
  grid.innerHTML=profiles.map(p=>{
    const shirts=(p.public_shirts||[]).slice(0,3);
    const nick=escapeHtml(p.nick||'?');
    return `<div class="card" style="cursor:pointer;padding:0;overflow:hidden;position:relative" onclick="viewProfile('${escJsAttr(p.nick||'')}')">
      ${adminMode?`<button onclick="event.stopPropagation();adminHideFromCommunity('${p.user_id}','${escJsAttr(p.nick||'')}')" style="position:absolute;top:8px;right:8px;z-index:2;background:rgba(192,57,43,.92);color:#fff;border:none;border-radius:6px;padding:4px 9px;font-size:9px;font-weight:700;font-family:'JetBrains Mono',monospace;letter-spacing:.04em;cursor:pointer;display:flex;align-items:center;gap:4px">${icon('trash',10)}${db.lang==='pl'?'ZDEJMIJ':'REMOVE'}</button>`:''}
      ${shirts.length?`<div style="display:grid;grid-template-columns:repeat(${shirts.length},1fr);height:110px;background:var(--bg3)">${shirts.map(s=>`<div style="overflow:hidden;display:flex;align-items:center;justify-content:center;background:var(--bg3)">${s.photo?`<img src="${escapeHtml(s.photo)}" style="width:100%;height:100%;object-fit:cover">`:icon('shirt',26,'opacity:.3')}</div>`).join('')}</div>`:`<div style="height:110px;background:var(--bg3);display:flex;align-items:center;justify-content:center;opacity:.3">${icon('shirt',32)}</div>`}
      <div style="padding:14px">
        <div class="fx-c gap12 mb8">
          <div class="profile-avatar" style="width:38px;height:38px;font-size:15px;flex-shrink:0">${nick[0].toUpperCase()}</div>
          <div style="flex:1;min-width:0">
            <div style="font-weight:800;font-size:14px;letter-spacing:.03em;line-height:1.3;margin-bottom:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${nick}${p.is_verified?verifiedBadgeHtml(p.nick,13):''}</div>
            <div style="font-size:9px;font-family:'JetBrains Mono',monospace;color:var(--t3);line-height:1.4">${p.shirt_count||0} ${db.lang==='pl'?'koszulek':'jerseys'}</div>
          </div>
        </div>
        <div class="fx-c gap6" style="color:var(--t2);font-size:11px">${icon('heart',13)}${likeCounts[p.user_id]||0}</div>
      </div>
    </div>`;
  }).join('');
}
async function adminHideFromCommunity(userId,nick){
  if(!adminMode)return;
  const reason=await customPrompt(db.lang==='pl'?`Powód zdjęcia profilu "${nick}" ze Społeczności (widoczny dla użytkownika):`:`Reason for removing "${nick}" from Community (visible to the user):`);
  if(reason===null)return;
  const{error}=await sb.from('profiles').update({visible:false}).eq('user_id',userId);
  if(error){toast((db.lang==='pl'?'Błąd: ':'Error: ')+error.message);return;}
  await createNotification(userId,'cancelled',
    db.lang==='pl'?'Twój profil został zdjęty ze Społeczności':'Your profile was removed from Community',
    db.lang==='pl'?`Powód: ${reason||'—'}. Możesz ponownie włączyć widoczność publiczną w ustawieniach profilu.`:`Reason: ${reason||'—'}. You can re-enable public visibility in your profile settings.`,
    'profile');
  toast(db.lang==='pl'?'Zdjęto profil ze Społeczności':'Profile removed from Community');
  initCommunityView();
}
async function loadProfileLikeCounts(userIds){
  if(!userIds.length)return{};
  const{data}=await sb.from('profile_likes').select('profile_user_id').in('profile_user_id',userIds);
  const counts={};
  (data||[]).forEach(r=>{counts[r.profile_user_id]=(counts[r.profile_user_id]||0)+1;});
  return counts;
}
async function toggleProfileLike(profileUserId){
  if(!currentUser){toast(db.lang==='pl'?'Zaloguj się, żeby polubić':'Log in to like this');return;}
  const{data:existing}=await sb.from('profile_likes').select('id').eq('profile_user_id',profileUserId).eq('user_id',currentUser.id).maybeSingle();
  if(existing){
    await sb.from('profile_likes').delete().eq('profile_user_id',profileUserId).eq('user_id',currentUser.id);
  } else {
    const{error}=await sb.from('profile_likes').insert({profile_user_id:profileUserId,user_id:currentUser.id});
    if(error){toast(error.message);return;}
    if(profileUserId!==currentUser.id){
      await createNotification(profileUserId,'wishlist',db.lang==='pl'?'Ktoś polubił Twoją kolekcję!':'Someone liked your collection!',db.profile?.nick?`${db.profile.nick} ${db.lang==='pl'?'polubił Twój profil':'liked your profile'}`:'','profile');
    }
  }
  refreshProfileLikeUI(profileUserId);
}
async function refreshProfileLikeUI(profileUserId){
  const{data:existing}=await sb.from('profile_likes').select('id').eq('profile_user_id',profileUserId).eq('user_id',currentUser?.id||'').maybeSingle();
  const counts=await loadProfileLikeCounts([profileUserId]);
  const btn=document.getElementById('profile-like-btn');
  if(!btn)return;
  const liked=!!existing;
  btn.classList.toggle('liked',liked);
  btn.innerHTML=`${icon('heart',14,liked?'fill:#c0392b;stroke:#c0392b':'')} ${counts[profileUserId]||0}`;
}
async function loadProfileComments(profileUserId){
  const{data,error}=await sb.from('profile_comments').select('*').eq('profile_user_id',profileUserId).order('created_at',{ascending:true}).limit(100);
  if(error){console.error('loadProfileComments error:',error);return[];}
  return data||[];
}
async function renderProfileComments(profileUserId){
  const list=document.getElementById('profile-comments-list');
  if(!list)return;
  const comments=await loadProfileComments(profileUserId);
  if(!comments.length){list.innerHTML=`<div style="font-size:11px;color:var(--t3);padding:10px 0">${db.lang==='pl'?'Brak komentarzy — bądź pierwszy':'No comments yet — be the first'}</div>`;return;}
  list.innerHTML=comments.map(c=>{
    const canDelete=currentUser&&(currentUser.id===c.user_id||adminMode);
    return `
    <div style="padding:9px 0;border-top:1px solid var(--border)">
      <div class="fx-b" style="margin-bottom:2px">
        <span style="font-size:11px;font-weight:700;color:var(--t1)">${escapeHtml(c.nick||'—')}</span>
        <span class="fx-c gap6">
          <span style="font-size:8px;font-family:'JetBrains Mono',monospace;color:var(--t3)">${c.created_at?new Date(c.created_at).toLocaleDateString('pl-PL'):''}</span>
          ${canDelete?`<button onclick="deleteProfileComment('${c.id}','${profileUserId}')" style="background:none;border:none;cursor:pointer;color:var(--t3);padding:0;display:flex" title="${db.lang==='pl'?'Usuń':'Delete'}">${icon('trash',11)}</button>`:''}
        </span>
      </div>
      <div style="font-size:12px;color:var(--t2);line-height:1.5">${escapeHtml(c.content)}</div>
    </div>`;
  }).join('');
}
async function deleteProfileComment(commentId,profileUserId){
  if(!await customConfirm(db.lang==='pl'?'Usunąć ten komentarz?':'Delete this comment?'))return;
  const{error}=await sb.from('profile_comments').delete().eq('id',commentId);
  if(error){toast((db.lang==='pl'?'Błąd: ':'Error: ')+error.message);return;}
  toast(db.lang==='pl'?'Usunięto komentarz':'Comment deleted');
  renderProfileComments(profileUserId);
}
async function postProfileComment(profileUserId){
  if(!currentUser){toast(db.lang==='pl'?'Zaloguj się, żeby skomentować':'Log in to comment');return;}
  const input=document.getElementById('profile-comment-input');
  const content=(input?.value||'').trim();
  if(!content){toast(db.lang==='pl'?'Wpisz treść komentarza':'Enter a comment');return;}
  const nick=db.profile?.nick||currentUser.email;
  const{error}=await sb.from('profile_comments').insert({profile_user_id:profileUserId,user_id:currentUser.id,nick,content});
  if(error){toast(error.message);return;}
  if(input)input.value='';
  renderProfileComments(profileUserId);
  if(profileUserId!==currentUser.id){
    await createNotification(profileUserId,'msg',db.lang==='pl'?'Nowy komentarz pod Twoim profilem':'New comment on your profile',`${nick}: ${content.slice(0,80)}`,'profile');
  }
}
