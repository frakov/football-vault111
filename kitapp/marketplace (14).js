...
async function openListingDetail(id){
  ...
  let photos=[l.photo].filter(Boolean);
  try{
    const{data:shirtRow}=await sb.from('shirts').select('photos').eq('id',id).maybeSingle();
    if(shirtRow?.photos?.length)photos=shirtRow.photos;
  }catch(e){}
    ...
    <div class="ld-layout">
      <div>
        <div class="ld-main-photo" id="ld-main-photo">
          ${photos.length?`<img src="${escapeHtml(photos[0])}" id="ld-main-img" alt="">`:`<div class="shirt-photo-placeholder" style="height:100%;display:flex;align-items:center;justify-content:center">${icon('shirt',52)}</div>`}
        ...
        ${photos.length>1?`<div class="ld-thumbs" id="ld-thumbs">${photos.map((p,i)=>`<div class="ld-thumb ${i===0?'active':''}" onclick="ldSetPhoto(${i})"><img src="${escapeHtml(p)}" alt=""></div>`).join('')}</div>`:''}
      </div>
      <div>
        <div class="ld-title">${escapeHtml(fullTitle||'—')}</div>

        <div class="ld-tags-row">
          ${l.season?`<span class="ld-tag">${db.lang==='pl'?'Sezon':'Season'}: <b>${escapeHtml(l.season)}</b></span>`:''}
          ${l.size?`<span class="ld-tag">${db.lang==='pl'?'Rozmiar':'Size'}: <b>${escapeHtml(l.size)}</b></span>`:''}
          ${l.brand?`<span class="ld-tag">${db.lang==='pl'?'Marka':'Brand'}: <b>${escapeHtml(l.brand)}</b></span>`:''}
          ${l.condition?`<span class="ld-tag" style="background:${condBg.bg};color:${condBg.fg}">${db.lang==='pl'?'Stan':'Cond.'}: <b>${l.condition}/10</b></span>`:''}
          ${l.personalization?`<span class="ld-tag">${escapeHtml(l.personalization)}</span>`:''}
        </div>
        ${l.legit_status==='approved'?`<div style="margin-top:12px" title="${db.lang==='pl'?'Ogłoszenie przeszło weryfikację autentyczności KitBase':'This listing passed KitBase authenticity verification'}">${legitStatusBadge('approved')}</div>`:''}

        <div class="ld-price-big" style="margin-top:18px">${mv?mv.toFixed(2)+' PLN':'—'}</div>
        ...
        ${!isOwn?`<button class="btn btn-mkt w100" style="margin-top:18px" onclick="contactSeller('${l.user_id||''}','${escJsAttr(l.seller_nick||'')}')">${icon('messageCircle',13,"display:inline-block;vertical-align:-2px;margin-right:6px")}${T('mkt_contact')}</button>`:''}
        ...
        <div class="ld-meta-row">
          <span>${icon('eye',12)} ${(l.views||0)+1} ${db.lang==='pl'?'wyświetleń':'views'}</span>
          <span>${icon('clock',12)} ${db.lang==='pl'?'Wystawiono':'Listed'} ${agoLabel(l.listed_at)}</span>
        </div>
