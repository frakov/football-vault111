...
function buildAuctionCard(a){
  ...
  return `<div class="mkt-card" data-auction-id="${a.id}" style="border-color:${isEnded?'var(--border)':'rgba(239,68,68,.25)'};background:${isEnded?'var(--bg2)':'rgba(239,68,68,.03)'}">
    <div class="shirt-photo" style="background:var(--bg3)">
      ...
      ${!isEnded?`<div style="position:absolute;bottom:8px;left:8px;right:8px;background:rgba(0,0,0,.75);border-radius:8px;padding:5px 8px;display:flex;justify-content:space-between;align-items:center;gap:6px;overflow:hidden">
        <div style="font-family:'JetBrains Mono',monospace;font-size:8px;color:rgba(255,255,255,.6);letter-spacing:.04em;flex-shrink:0">${db.lang==='pl'?'KOŃCZY SIĘ':'ENDS IN'}</div>
        <div id="${timerId}" style="font-family:'Inter',sans-serif;font-weight:800;font-size:13px;color:var(--a);letter-spacing:.02em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${formatCountdown(a.ends_at)}</div>
      </div>`:''}
    </div>
