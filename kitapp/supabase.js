...
async function sbLoadData(){
  ...
  db.shirts=(shirts||[]).map(r=>({
    ...
    styleCode:r.style_code, buyPrice:r.buy_price, marketValue:r.market_value, listPrice:r.list_price||null, notes:r.notes,
...
pl:{
  ...
  nav_market:'Marketplace',nav_profile:'Profil',community:'Społeczność',
  ...
  f_photos:'Zdjęcia',f_status:'Status',f_brand:'Marka',
...
en:{
  ...
  nav_market:'Marketplace',nav_profile:'Profile',community:'Community',
  ...
  f_photos:'Photos',f_status:'Status',f_brand:'Brand',
...
// Sezony od najnowszego do 1980/81 — najnowszy na górze, bo większość koszulek jest świeża
const SEASONS=(()=>{const arr=[];const startYear=2026;for(let y=startYear;y>=1980;y--){arr.push(`${y}/${String((y+1)%100).padStart(2,'0')}`);}return arr;})();
...
const APPS=[
  ...
  {id:'community', lpl:'Społeczność', len:'Community', icon:'users',  group:'Handel'},
...
const _postOpen={
  ...
  community:()=>setTimeout(()=>initCommunityView(),50),
};
...
// ── DASHBOARD ──
function closeModal(){document.getElementById('modal-overlay').classList.remove('open');document.getElementById('modal-box')?.classList.remove('modal-fullscreen');editId=null;editPhotos=[];editPhotoFiles=[];}
