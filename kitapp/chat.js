  ...
  el.innerHTML=msgs.map(m=>{
    ...
    return `<div class="chat-msg ${isMe?'me':'them'}" data-msg-id="${m.id}">
      ...
      <div class="chat-bubble ${isMe?'mine':'theirs'}" style="position:relative">
        ${isMe||adminMode?`<button class="msg-del-btn" onclick="deleteMessage('${m.id}','${convId}')" title="${db.lang==='pl'?'Usuń':'Delete'}">${icon('trash',10)}</button>`:''}
...
async function deleteMessage(msgId,convId){
  ...
  let q=sb.from('messages').update({deleted:true,content:'',image_url:null}).eq('id',msgId);
  if(!adminMode)q=q.eq('sender_id',currentUser.id);
  const{error}=await q;
