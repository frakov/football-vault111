...
async function initCommunityView(){
  ...
  grid.innerHTML=profiles.map(p=>{
    ...
    return `<div class="card" style="cursor:pointer;padding:0;overflow:hidden" onclick="viewProfile('${escJsAttr(p.nick||'')}')">
      ${shirts.length?`<div style="display:grid;grid-template-columns:repeat(${shirts.length},1fr);height:110px;background:var(--bg3)">${shirts.map(s=>`<div style="overflow:hidden">${s.photo?`<img src="${escapeHtml(s.photo)}" style="width:100%;height:100%;object-fit:cover">`:''}</div>`).join('')}</div>`:`<div style="height:110px;background:var(--bg3);display:flex;align-items:center;justify-content:center;opacity:.3">${icon('shirt',32)}</div>`}
