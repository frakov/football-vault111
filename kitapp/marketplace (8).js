...
async function openListingDetail(id){
  ...
  document.getElementById('modal-body').innerHTML=`
    <div class="ld-layout">
      ...
      <div>
        <div class="ld-card">
          <div class="fx-b" style="align-items:flex-start;position:relative;z-index:1">
            <div>
              <div class="ld-card-club">${escapeHtml(l.club||'—')}</div>
              <div class="ld-card-sub">${escapeHtml([l.season,l.kind].filter(Boolean).join(' · '))}</div>
            </div>
            ${likeButtonHtml(l.id,'lg')}
          </div>
          ${l.legit_status==='approved'?`<div style="margin-top:10px;position:relative;z-index:1">${legitStatusBadge('approved')}</div>`:''}
          <div class="ld-card-grid">
            <div><span class="ld-card-label">${db.lang==='pl'?'Sezon':'Season'}</span><span class="ld-card-val">${escapeHtml(l.season||'—')}</span></div>
            <div><span class="ld-card-label">${db.lang==='pl'?'Marka':'Brand'}</span><span class="ld-card-val">${escapeHtml(l.brand||'—')}</span></div>
            <div><span class="ld-card-label">${db.lang==='pl'?'Rodzaj':'Kit type'}</span><span class="ld-card-val">${escapeHtml(l.kind||'—')}</span></div>
            <div><span class="ld-card-label">${db.lang==='pl'?'Rozmiar':'Size'}</span><span class="ld-card-val">${escapeHtml(l.size||'—')}</span></div>
            <div><span class="ld-card-label">${db.lang==='pl'?'Stan':'Condition'}</span><span class="ld-card-val" style="color:${l.condition?ldCondColor(l.condition):'#fff'}">${l.condition?l.condition+'/10':'—'}</span></div>
            <div><span class="ld-card-label">${db.lang==='pl'?'Cena':'Price'}</span><span class="ld-card-val">${mv?mv.toFixed(2)+' PLN':'—'}</span></div>
            ${l.personalization?`<div style="grid-column:1/-1"><span class="ld-card-label">${db.lang==='pl'?'Personalizacja':'Personalization'}</span><span class="ld-card-val">${escapeHtml(l.personalization)}</span></div>`:''}
          </div>
          ${l.notes?`<div class="ld-card-notes"><span class="ld-card-label">${db.lang==='pl'?'Opis':'Description'}</span><div>${escapeHtml(l.notes)}</div></div>`:''}
          <div class="ld-card-footer">${db.lang==='pl'?'Wystawiono':'Listed'} ${agoLabel(l.listed_at)}</div>
        </div>

        <div class="ld-seller" onclick="closeModal();viewProfile('${escJsAttr(l.seller_nick||'')}')">
          <div>
            <div class="ld-seller-label" style="margin-bottom:6px">${db.lang==='pl'?'Z kolekcji':'From collection'}</div>
            <div class="fx-c gap8">
              <div class="ld-avatar">${escapeHtml((l.seller_nick||'?')[0].toUpperCase())}</div>
              <span style="font-size:13px;font-weight:700;color:var(--t1)">${escapeHtml(l.seller_nick||'—')}</span>${verifiedBadgeHtml(l.seller_nick,14)}
            </div>
          </div>
          <div style="font-size:10px;color:var(--t3);font-family:'JetBrains Mono',monospace">${(l.views||0)+1} ${db.lang==='pl'?'wyświetleń':'views'}</div>
        </div>
        ...
        <div class="fx-c gap8" style="margin-top:14px;flex-wrap:wrap">
          <button class="btn btn-g" style="flex:1" onclick="shareListing('${l.id}')">${db.lang==='pl'?'Udostępnij':'Share'}</button>
          ${!isOwn?`<button class="btn btn-g" style="flex:1;opacity:.75" onclick="reportListing('${l.id}')">${flagIcon}${db.lang==='pl'?'Zgłoś':'Report'}</button>`:''}
        </div>
